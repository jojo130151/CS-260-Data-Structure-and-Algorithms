...
void HashTable::Insert(Bid bid) {
	...
	if (oldNode == nullptr) {
		...
		if (oldNode->key == UINT_MAX) {
			...
			// Find at next open node (last one)
...
void HashTable::PrintAll() {
    for (int i = 0; i < tableSize; ++i) {            // Iterate through all keys present in vector
		Node* node = &(nodes.at(i));                 // Retrieve node belonging to the key, represented by i in for loop
		if (node != nullptr && node->key != UINT_MAX) {
			cout << "Key " << node->key << ": " << node->bid.bidId << " | " << node->bid.title << " | " << node->bid.amount << " | "
		         << node->bid.fund << endl;
			node = node->next;                       // Iterate through list at current key
		    while (node != nullptr) {
